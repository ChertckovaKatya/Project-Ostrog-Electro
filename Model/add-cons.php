<?php
	...
	function add_object($owner_fio,$renter_fio,$name_object,$mail_address,$sourse_of_power,$vol_class,$date_check,$user_id)
		{
			include_once "..\Setting\connection.php";
			$connect = get_connect();
			if (!empty($owner_fio) AND !empty($renter_fio) AND !empty($name_object) AND !empty($mail_address)  AND !empty($sourse_of_power) AND !empty($vol_class) AND !empty($date_check)  AND !empty($user_id) )
			{
  				...
  				VALUES ('$owner_fio','$renter_fio','$name_object','$mail_address','$sourse_of_power','$vol_class','$date_check','$user_id');");
